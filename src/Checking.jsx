...
function Checking() {
  ...
  const [emailReplyTo, setEmailReplyTo] = useState(() => {
    const v =
      ENV_EMAIL_REPLY_TO && String(ENV_EMAIL_REPLY_TO).trim()
        ? String(ENV_EMAIL_REPLY_TO).trim()
        : "";
    return v;
  });
  ...
  const [senderEmail, setSenderEmail] = useState(() => {
    const v =
      ENV_EMAIL_FROM && String(ENV_EMAIL_FROM).trim()
        ? String(ENV_EMAIL_FROM).trim()
        : "";
    return v;
  });
  ...
  const buildStableResultDedupeKey = (type, item) => {
    const statusKey = toComparableString(item?.microdegreeStudentStatus)
      .trim()
      .toLowerCase();
  ...
  const getUniquePhoneNumbers = (phone1, phone2) => {
    ...
    const addIfValid = (p) => {
      ...
      const normalized =
        normalizePhoneNumber(cleaned) || stripToDigits(cleaned);
  ...
  const validateForm = () => {
    ...
    if (
      ...
      !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.alternateEmail.trim())
    ) {
      errors.alternateEmail = "Enter a valid alternate email";
    }
    if (
      formData.alternatePhone.trim() &&
      !isValidPhoneSearchValue(formData.alternatePhone.trim())
    ) {
  ...
  const handleFormSubmit = async (event) => {
    ...
    const payload = {
      ...
      course_fee: formData.courseFee.trim()
        ? Number(formData.courseFee.trim())
        : null,
    ...
    if (type === "email") {
      ...
      if (normalizedCoreNumber.length === 10) {
        ...
        searchConditions.add(
          `alternate_phone.eq.${"+1" + normalizedCoreNumber}`,
        );
        ...
        searchConditions.add(
          `alternate_phone.eq.${"+91" + normalizedCoreNumber}`,
        );
        ...
        searchConditions.add(
          `alternate_phone.eq.${"0" + normalizedCoreNumber}`,
        );
      }
  ...
  const handleSearch = async (type, inputOverride) => {
    ...
    const inputToUse =
      typeof inputOverride === "string" ? inputOverride : searchInput;
    const valuesToSearch = Array.isArray(inputOverride)
      ? inputOverride
      : parseInputValues(inputToUse);
    ...
    if (type === "phone") {
      ...
      if (validPhoneValues.length === 0) {
        setValidationMessage(
          "Please enter at least one valid phone number (min 10 digits).",
        );
        setResults([]);
        return;
      }
      ...
      if (type === "phone") {
        ...
        for (const value of valuesToSearch) {
          ...
          if (data && data.length > 0) {
            data.forEach((foundItem) => {
              const fullName =
                `${foundItem.first_name || ""} ${foundItem.last_name || ""}`.trim();
              const courseFees =
                foundItem.course_fee !== null ? foundItem.course_fee : "N/A";
              const primaryEmail =
                foundItem.email || foundItem.alternate_email || "N/A";
        ...
        valuesToSearch.forEach((v) => {
          ...
          const foundInResults = uniqueResults.some(
            (r) =>
              buildPhoneKey(r.foundPhone || "") === key ||
              buildPhoneKey(r.value || "") === key,
          );
        ...
        if (missingNumbers.length > 0) {
          ...
          setValidationMessage(
            missingNumbers.length === 1
              ? `Number not found: ${list}`
              : `Numbers not found: ${list}`,
          );
        } else {
          setValidationMessage("");
        }
      ...
      if (type === "email") {
        ...
        const missingEmails = valuesToSearch.filter(
          (val) =>
            !dedupedByEmail.some(
              (r) =>
                (r.value || "").toLowerCase() === (val || "").toLowerCase() &&
                r.microdegreeStudentStatus === "Yes student found",
            ),
        );
        if (missingEmails.length > 0) {
          ...
          setValidationMessage(
            missingEmails.length === 1
              ? `Email not found: ${list}`
              : `Emails not found: ${list}`,
          );
        } else {
          setValidationMessage("");
        }
  ...
  const getFilteredResults = () => {
    const statusFiltered =
      filterStatus === "All"
        ...
        : results.filter(
            (item) =>
              (item.microdegreeStudentStatus || "").trim().toLowerCase() ===
  ...
  const applyCourseFilters = (items) => {
    ...
    return items.filter((item) => {
      if (
        courseFilters.coursePlan &&
        courseFilters.coursePlan.length > 0 &&
        !courseFilters.coursePlan.includes(item.coursePlan)
      )
        return false;

      if (
        courseFilters.location &&
        courseFilters.location.length > 0 &&
        !courseFilters.location.includes(item.location)
      )
        return false;

      if (
        courseFilters.courseType1 &&
        courseFilters.courseType1.length > 0 &&
        !courseFilters.courseType1.includes(item.courseType1)
      )
        return false;
  ...
  const hasActiveJoiningFilters = Boolean(
    joiningDateRange.from || joiningDateRange.to,
  );
  ...
  const visibleYesCount = filteredResults.filter(
    (r) => r.microdegreeStudentStatus === "Yes student found",
  ).length;
  const visibleNoCount = filteredResults.filter(
    (r) => r.microdegreeStudentStatus === "Not a student",
  ).length;
  ...
  if (courseFilters.coursePlan && courseFilters.coursePlan.length > 0) {
    activeFilterParts.push(
      `Course Plan (${courseFilters.coursePlan.join(", ")})`,
    );
  }
  if (courseFilters.courseType1 && courseFilters.courseType1.length > 0) {
    activeFilterParts.push(
      `Delivery Mode (${courseFilters.courseType1.join(", ")})`,
    );
  }
  ...
  const resetCourseFilters = () =>
    setCourseFilters({ coursePlan: [], location: [], courseType1: [] });
  ...
  const sendBulkEmail = async () => {
    ...
    try {
      ...
      const resultsArr = settled.map((s) =>
        s.status === "fulfilled"
          ? s.value
          : { to: "unknown", success: false, error: "send promise rejected" },
      );
      ...
      const allOk =
        resultsArr.length > 0 && resultsArr.every((r) => r.success === true);
      if (allOk) {
        ...
        setEmailSendStatus({
          type: "error",
          text: "Failed to send some or all emails.",
        });
        alert(
          "Failed to send some or all emails. See details in the results window.",
        );
      }
    } catch (err) {
      ...
      <div style={filterToggleBarStyle}>
        ...
        {results.length > 0 && (
          <div style={filterToggleSummaryCenterStyle}>
            Visible: {filteredResults.length} | Yes: {visibleYesCount} | No:{" "}
            {visibleNoCount}
      ...
      <div
        style={
          areFiltersOpen
            ...
            : { ...mainContentWrapperStyle, gap: "0px" }
        }
      >
        <div
          style={
        ...
        <div className="table-column" style={resultsPanelWrapperStyle}>
          {isFetching ? (
                ...
                <tbody>
                  {filteredResults.map((item, index) => {
                    ...
                    const showEmailInCoursePlan = Boolean(
                      item.foundEmail &&
                      !["N/A", "Error", "Not Found"].includes(
                        String(item.foundEmail),
                      ),
                    );
                    const showPhoneInCoursePlan = Boolean(
                      item.foundPhone &&
                      !["N/A", "Error", "Not Found"].includes(
                        String(item.foundPhone),
                      ),
                    );
                    return (
                      <tr
                      ...
                      >
                        <td style={tdStyle}>
                          {item.foundEmail &&
                          item.foundEmail !== "N/A" &&
                          item.foundEmail !== "Error" &&
                          item.foundEmail !== "Not Found" ? (
                            <input
                              type="checkbox"
                              checked={selectedEmails.has(item.foundEmail)}
                              onChange={() =>
                                toggleSelectEmail(item.foundEmail)
                              }
                            ...
                            <input
                              ...
                              style={{ ...checkboxStyle, opacity: 0.45 }}
                            />
                          )}
                        </td>
                        <td style={tdStyle}>
                          ...
                          {item.value !== "N/A" &&
                            ...
                            item.value !== "Not Found" && (
                              <button
                                onClick={() =>
                                  copyToClipboard(item.value, index, "searched")
                                }
                        ...
                        <td style={{ ...tdStyle, whiteSpace: "normal" }}>
                          ...
                          >
                            ...
                            {(showEmailInCoursePlan ||
                              showPhoneInCoursePlan) && (
                              <div
                                style={{
                                  fontSize: "12px",
                                  color: "#64748b",
                              ...
                              >
                                {showEmailInCoursePlan && (
                                  <div>Email: {item.foundEmail}</div>
                                )}
                                {showPhoneInCoursePlan && (
                                  <div>Phone: {item.foundPhone}</div>
                                )}
                              </div>
                            )}
                          </div>
                        ...
                        {activeSearchType === "email" && (
                          <td style={tdStyle}>
                            {item.foundPhone}
                            {item.foundPhone !== "N/A" &&
                              item.foundPhone !== "Error" &&
                              item.foundPhone !== "Not Found" && (
                                <button
                                  onClick={() =>
                                    copyToClipboard(
                                      item.foundPhone,
            ...
            <div style={emptyResultsStyle}>
              Start a search to see student data on the right.
            </div>
          )}
        </div>
      </div>
      ...
      {results.length > 0 && selectedEmails.size > 0 && (
        <div style={selectionBarStyle}>
          ...
          <div style={{ color: "#64748b" }}>
            Select recipients for bulk email
          </div>
              ...
              <div style={formGridStyle}>
                <div style={formFieldStyle}>
                  ...
                  {formErrors.firstName && (
                    <span style={errorTextStyle}>{formErrors.firstName}</span>
                  )}
                </div>
                ...
                <div style={formFieldStyle}>
                  ...
                  />
                  {formErrors.lastName && (
                    <span style={errorTextStyle}>{formErrors.lastName}</span>
                  )}
                </div>
                ...
                <div style={formFieldStyle}>
                  ...
                  />
                  {formErrors.phone && (
                    <span style={errorTextStyle}>{formErrors.phone}</span>
                  )}
                </div>
                ...
                <div style={formFieldStyle}>
                  ...
                  >
                    ...
                    <option value="GenAi Premium">GenAi Premium</option>
                    <option value="GenAi Premium Plus">
                      GenAi Premium Plus
                    </option>
                ...
                >
                  ...
                  >
                    ...
                    <input
                      ...
                      placeholder={
                        toEmailPills.length === 0
                          ? "Type an email, press Enter or comma"
                          : ""
                      }
                      style={{
                        flex: "1 1 180px",
          ...
          <div style={{ ...modalContentStyle, maxWidth: "720px" }}>
            ...
            <div style={{ maxHeight: "60vh", overflowY: "auto" }}>
              {sendResults.length === 0 && (
                <div>No send results available.</div>
              )}
          ...
          <div style={{ ...modalContentStyle, maxWidth: "720px" }}>
            ...
            <div style={{ maxHeight: "60vh", overflowY: "auto" }}>
              ...
              {!isDebugging && debugResults.length === 0 && (
                <div>No debug results yet. Click Debug Fetch.</div>
              )}
              {debugResults.map((r, idx) => (
                <div
                  key={idx}
                  style={{ padding: "8px 6px", borderBottom: "1px solid #eee" }}
                >
                  <div style={{ fontWeight: "bold" }}>{r.value}</div>
                  <div
                    style={{
                      fontSize: "0.9rem",
                      color: r.error ? "#dc3545" : "#28a745",
                    }}
                  >
